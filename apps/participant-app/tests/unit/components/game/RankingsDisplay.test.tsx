...
import { RankingsDisplay } from '@/components/game/RankingsDisplay';
...
describe('RankingsDisplay', () => {
  ...
  describe('Non-final questions (isGongActive: false)', () => {
    ...
    it('should highlight current guest in worst 10', () => {
      render(<RankingsDisplay results={results} isGongActive={false} currentGuestId="g4" />);
  ...
  describe('Period-final questions (isGongActive: true)', () => {
    ...
    it('should highlight current guest in top 10', () => {
      render(<RankingsDisplay results={results} isGongActive={true} currentGuestId="g2" />);
  ...
  describe('Formatting', () => {
    ...
    it('should show "You" indicator for current guest', () => {
      const results: GameResults = {
        top10: mockTop10,
        worst10: mockWorst10,
      };

      render(<RankingsDisplay results={results} isGongActive={true} currentGuestId="g1" />);

      const youIndicator = screen.getByText('(You)');
      expect(youIndicator).toBeInTheDocument();
      expect(youIndicator).toHaveClass('text-blue-600');
    });
  });
});
